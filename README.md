...
> 当前发布版本：`v0.1.13`。已在 Debian 13、Arch/Omarchy 和 openEuler 24.03 上完成真实回归，包含多窗口、多用户共享隧道、断线恢复和 30 分钟连续观察。默认不安装远端常驻 Agent、不开放公网端口、不保存密码或 API Token。
...
也可以从 [v0.1.13 Release](https://github.com/zylona/re_remote_dev/releases/tag/v0.1.13) 下载完整本地集成制品。生产环境建议固定版本并校验 SHA256：
...
```bash
./remote-dev-orchestrator-v0.1.13-linux/rollback 0.1.10
```
